...
def isSubstrOf (target str : String) : Bool :=
  str.containsSubstr target
...
def filterUserPremises (premises : Multiset Name) (proofSource : String)
  ...
  let mut result := Lean.RBMap.empty
  for (p, c) in premises do
    let pLast := (Syntax.splitNameLit p.toString.toRawSubstring).reverse.head!.toString
